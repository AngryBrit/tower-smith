...
| `tables/` | Screenshot-derived **GOD** lab/workshop tables (authoritative cost/value ground truth). Labs: [`src/data/labGodTables.ts`](src/data/labGodTables.ts) + [`src/labCosts.ts`](src/labCosts.ts). Workshop: [`src/data/workshopGodTables.ts`](src/data/workshopGodTables.ts) + [`src/workshopCosts.ts`](src/workshopCosts.ts). Audit labs with `node scripts/audit-lab-god-sources.mjs`. |
