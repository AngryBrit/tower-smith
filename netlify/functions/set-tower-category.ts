...
import { updateTowerCategory } from './lib/supabaseGallery'
...
export default async (req: Request): Promise<Response> => {
  ...
  const category =
    typeof categoryRaw === 'string' && isGalleryBuildCategory(categoryRaw.trim())
      ? categoryRaw.trim()
      : null
  ...
  if (!category) {
    return jsonResponse(400, { error: 'invalid_category' }, cors)
  }

  const entry = await updateTowerCategory(id, user, category)
  if (!entry) {
