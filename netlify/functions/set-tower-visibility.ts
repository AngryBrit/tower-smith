...
import { updateTowerVisibility } from './lib/supabaseGallery'

function parseVisibility(raw: unknown): 'public' | 'unlisted' | null {
...
export default async (req: Request): Promise<Response> => {
  ...
  const visibility = parseVisibility((raw as { visibility?: unknown }).visibility)
  if (!isGalleryBuildId(id)) {
    return jsonResponse(400, { error: 'invalid_id' }, cors)
  }
  if (!visibility) {
    return jsonResponse(400, { error: 'invalid_visibility' }, cors)
  }

  const entry = await updateTowerVisibility(id, user, visibility)
  if (!entry) {
