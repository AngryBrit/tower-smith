...
import { lookupGuardiansWorkbookOnIdsGrid } from '../../../src/effectivePaths/lookupGuardiansOnIdsGrid'
...
export async function readIdsGatewayLookup(options: {
  ...
  const botsFromRow = lookupBotsWorkbookOnIdsGrid(grid)
  const botsWorkbook =
    botsFromRow && filterKnownIdsWorkbooks([botsFromRow])[0]
      ? botsFromRow
      : findBotsWorkbook(workbooks)
  const laboratoryFromRow = lookupLaboratoryWorkbookOnIdsGrid(grid)
  const laboratoryWorkbook =
    laboratoryFromRow && filterKnownIdsWorkbooks([laboratoryFromRow])[0]
      ? laboratoryFromRow
  ...
  const guardiansFromRow = lookupGuardiansWorkbookOnIdsGrid(grid)
  const guardiansWorkbook = guardiansFromRow ?? findGuardiansWorkbook(workbooks)
...
export async function resolveBotsWorkbookId(options: {
  accessToken: string
  masterSpreadsheetId: string
  sheetGid: number | null
}): Promise<string> {
  const gateway = await readIdsGatewayLookup(options)
  if (!gateway.botsWorkbook) {
    throw new GoogleSheetsApiError('sheets_api_error', 404, 'bots_workbook_not_found')
  }
  return gateway.botsWorkbook.spreadsheetId
}

export async function resolveLaboratoryWorkbookId(options: {
  accessToken: string
  masterSpreadsheetId: string
  sheetGid: number | null
}): Promise<string> {
  const gateway = await readIdsGatewayLookup(options)
  if (!gateway.laboratoryWorkbook) {
    throw new GoogleSheetsApiError('sheets_api_error', 404, 'laboratory_workbook_not_found')
  }
  return gateway.laboratoryWorkbook.spreadsheetId
}

export async function resolveUwsWorkbookId(options: {
  accessToken: string
  masterSpreadsheetId: string
  sheetGid: number | null
}): Promise<string> {
  const gateway = await readIdsGatewayLookup(options)
  if (!gateway.uwsWorkbook) {
    throw new GoogleSheetsApiError('sheets_api_error', 404, 'uws_workbook_not_found')
  }
  return gateway.uwsWorkbook.spreadsheetId
}

export async function resolveGuardiansWorkbookId(options: {
  accessToken: string
  masterSpreadsheetId: string
  sheetGid: number | null
}): Promise<string> {
  const gateway = await readIdsGatewayLookup(options)
  if (!gateway.guardiansWorkbook) {
    throw new GoogleSheetsApiError('sheets_api_error', 404, 'guardians_workbook_not_found')
  }
  return gateway.guardiansWorkbook.spreadsheetId
}
