...
/** Wiki table: B/T/q/Q → 1e9 / 1e12 / 1e15 / 1e18 */
function parseCost(s) {
  s = s.trim()
  const m = /^([\d.]+)\s*([bBtTqQ])$/.exec(s)
...
for (const line of rows) {
  const [lv, dur, cost] = line.split('\t')
  const d = parseDur(dur)
  const c = parseCost(cost)
  levels.push({ lv: parseInt(lv, 10), d, c })
  sumCost += c
}
...
for (const x of levels) {
  fragLines.push(`    "${x.lv}": {`)
  fragLines.push(`      "DURATION": ${x.d},`)
  fragLines.push(`      "COST": ${x.c}`)
  fragLines.push(`    }${x.lv < 100 ? ',' : ''}`)
}

function labBlock(name) {
  return [`  "${name}": {`, ...fragLines, '  }'].join('\n')
}
...
const out = names.map((n) => labBlock(n)).join(',\n')
const outPath = path.join(__dirname, '..', 'tmp-enhancement-labs-insert.json')
fs.writeFileSync(outPath, out + '\n', 'utf8')

const metaPath = path.join(__dirname, '..', 'tmp-enhancement-labs-meta.json')
fs.writeFileSync(
  metaPath,
  JSON.stringify({ coinsToMaxRaw: sumCost, level1: levels[0], level100: levels[99] }, null, 2),
  'utf8',
)
