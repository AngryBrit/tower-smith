...
/**
 * Find the Themes & Songs **workbook** spreadsheet ID on the IDS gateway tab.
 */
export function lookupThemesWorkbookOnIdsGrid(
  ...
  for (let rowIndex = 0; rowIndex < formatted.length; rowIndex++) {
    ...
    for (let col = 0; col < maxCols; col++) {
      ...
      if (!cell) continue
      if (isThemesWorkbookName(cell)) {
        themesRow = true
        name = cleanEffectivePathsCategoryName(cell)
        break
      }
      if (/go to my themes?\b/i.test(cell) || /themes?\s*&\s*songs/i.test(cell)) {
        themesRow = true
        name = name || 'Themes & Songs'
      }
    }

    if (!themesRow) continue
    ...
    return { name: name || 'Themes & Songs', spreadsheetId }
  }
