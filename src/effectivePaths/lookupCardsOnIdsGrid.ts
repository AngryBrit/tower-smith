...
/** Find the Cards workbook spreadsheet ID on the IDS gateway tab. */
export function lookupCardsWorkbookOnIdsGrid(
  ...
  for (let rowIndex = 0; rowIndex < formatted.length; rowIndex++) {
    ...
    for (let col = 0; col < maxCols; col++) {
      ...
      if (!cell) continue
      if (isCardsWorkbookName(cell)) {
        cardsRow = true
        name = cleanEffectivePathsCategoryName(cell)
        break
      }
      if (/go to my cards?\b/i.test(cell)) {
        cardsRow = true
        name = name || 'Cards'
      }
    }
    ...
    return { name: name || 'Cards', spreadsheetId }
  }
