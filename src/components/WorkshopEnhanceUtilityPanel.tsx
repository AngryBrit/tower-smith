...
function WorkshopEnhanceUtilityCard({
  ...
  const nextCoins = discountedWorkshopBulkMarginal(
    ...
    max,
    bulkStep,
    (L) => workshopEnhanceUtilityNextMarginalCoins(upgradeKey, L),
    coinDiscountPercent,
  )
  ...
  const bump = useCallback(
    (key: WorkshopEnhanceUtilityUpgradeKey, direction: -1 | 1) => {
      ...
      const nv = workshopEnhanceUtilityClampLevel(
        key,
        level +
          workshopBulkBumpDelta(direction, multiplier, level, workshopEnhanceUtilityMaxLevel(key)),
      )
        ...
        return (
          <WorkshopEnhanceUtilityCard
            ...
            onSetLevel={(n) =>
              onWorkshopPersistedChange({
                ...workshopPersisted,
                [key]: workshopEnhanceUtilityClampLevel(key, n),
              })}
            bulkStep={multiplier}
