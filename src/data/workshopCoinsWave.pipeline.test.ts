...
describe.skipIf(!existsSync(PLAYER_SAVE))('workshopCoinsWave pipeline', () => {
  it('calibrates coins per wave display against in-game', async () => {
    ...
    const level = ws.coinsWaveLevel
    const workshop = workshopCoinsWaveStatAmount(level)
    const labMult = opts?.coinsWaveLabMultiplier ?? 1
    const display = workshopUtilityStatDisplay('coinsWaveLevel', level, opts)

    expect(labMult).toBeCloseTo(lab?.coinsWaveLabMultiplier ?? 1, 2)
    expect(Math.round(workshop * labMult)).toBe(228)
    expect(display).toBe('228')
  })
})
